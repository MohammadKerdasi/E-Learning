...
export default function CardOpenPage({card}: {card:items}) {
  return (
    <div className="bg-white rounded-xl p-7.5 lg:p-[6.35%] 1600:p-[50px]">
      <TitleAsNumber numberB={card.num} />
      <div className="py-7.5 lg:py-[40px] 1600:py-[50px]">
      ...
      <div className="flex flex-col gap-5">
        ...
        return (
          <div key={un} className={`flex justify-start sm:justify-between gap-y-[24px] sm:items-center flex-col sm:flex-row p-[24px] lg:px-[4.364%] lg:py-5 1600:px-7.5 1600:py-[24px] rounded-md border ${isConditionMet ? 'border-Orange/80 shadow-videoLK' : 'border-White/97'}`}>
            <div className="w-[67%]">
              <h3 className="text-base font-medium desktop:text-xl desktop:leading-7.5 text-Grey/20 pb-[6px]">{lessons.title}</h3>
              <h5 className="desktop:text-lg desktop:leading-6.5 font-normal text-sm text-Grey/35">{lessons.lessonNum}</h5>
            </div>
            <div className={`flex justify-center items-center gap-[2px] w-max whitespace-nowrap p-2.5 rounded-md ${isConditionMet ? 'bg-Orange/90' : ' bg-White/97'}`}>
